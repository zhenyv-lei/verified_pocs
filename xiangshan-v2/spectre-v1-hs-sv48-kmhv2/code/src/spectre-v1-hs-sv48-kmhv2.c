...
static void run_victim_process(uint64_t idx, volatile uint8_t *probe)
{
  ...
  mstatus = (mstatus & ~MSTATUS_MPP_MASK) | (1ull << 11);
                     ...
                     uint64_t a0, uint64_t a1, uint64_t a7)
{
  ...
  if (cause != MCAUSE_SUPERVISOR_ECALL) {
    bad_trap_count++;
    attack_status = 2;
    attack_done = 1;
    return;
...
static void enter_attacker_process(void)
{
  ...
  mstatus = (mstatus & ~MSTATUS_MPP_MASK) | (1ull << 11);
#if SMOKE_TRACE
  printf("[hs] pre satp=%lx\n", observed_attacker_satp);
#endif
...
int main(void)
{
...
#if !SMOKE_FAST_BOOT
  printf("[v1-hs-sv48] model=sv48-asid-hs-isolation attacker=HS victim=HS service=machine-scheduler attacker_asid=%lu victim_asid=%lu normalized_attacker_asid=%lu normalized_victim_asid=%lu fence_on_switch=%d baseline_subtract=%d filter_known_noise=%d secret_sz=%d candidates=%d flush_lines=%d control_only=%d\n",
  ...
  printf("[v1-hs-sv48] secret_offset=%d\n", SECRET_OFFSET);
  printf("[v1-hs-sv48] calibration fallback=%d measured=%lu threshold=%lu fixed=%d\n",
         CACHE_HIT_THRESHOLD, measured_threshold, active_threshold,
         USE_FIXED_CACHE_HIT_THRESHOLD);
  printf("[v1-hs-sv48] isolation attacker_direct_victim_secret=expect-fault shared_probe=mapped-both victim_data=mapped-victim-only\n");
  printf("[v1-hs-sv48] victim_array1=%p victim_secret=%p malicious_base=%lu probe=%p\n",
  ...
  printf("[v1-hs-sv48] isolation HS-direct-victim-read fault_seen=%lu completed=%lu value=0x%02lx u_text=[%p,%p) a_data=[%p,%p) shared=[%p,%p) v_data=[%p,%p)\n",
  ...
  printf("[v1-hs-sv48] satp requested_attacker=0x%lx requested_victim=0x%lx observed_attacker=0x%lx observed_victim=0x%lx asid_mask=0x%lx mode=sv48\n",
         attacker_space.satp, victim_space.satp,
         observed_attacker_satp, observed_victim_satp, SATP_ASID_MASK);
  printf("[v1-hs-sv48] traps=%lu victim_calls=%lu victim_done=%lu flush_calls=%lu exit_calls=%lu bad_traps=%lu last_mcause=%lu last_mepc=%p status=%d\n",
  ...
  for (uint64_t i = 0; i < SECRET_SZ; ++i) {
    ...
    printf("[v1-hs-sv48] byte %lu: expected=%c(0x%02x) guess=%c(0x%02x) score=%lu second=%c(0x%02x) score=%lu\n",
           i,
           expected, expected,
           (guess >= 32 && guess < 127) ? guess : '?', guess, leaked_score[i][0],
           (second >= 32 && second < 127) ? second : '?', second, leaked_score[i][1]);
    printf("[v1-hs-sv48] byte %lu rounds=%lu early_stop=%d min_score=%d gap=%d\n",
